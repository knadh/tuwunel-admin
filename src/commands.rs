...
mod tests {
    ...
    fn render_positional() {
        let cmd = find("users", "create").unwrap();
        let out = render_template(cmd, &vals(&[("username", "alice"), ("password", "pw")]));
        assert_eq!(out, "users create-user alice pw");
    }

    #[test]
    fn render_optional_flag_with_value() {
        let cmd = find("users", "last-active").unwrap();
        let out = render_template(cmd, &vals(&[("limit", "50")]));
        assert_eq!(out, "users last-active --limit 50");
        let out = render_template(cmd, &vals(&[]));
        assert_eq!(out, "users last-active");
    }

    #[test]
    fn render_bool_flag() {
        let cmd = find("users", "deactivate").unwrap();
        let out = render_template(cmd, &vals(&[("user_id", "@a:s"), ("no_leave_rooms", "on")]));
        assert_eq!(out, "users deactivate @a:s --no-leave-rooms");
        let out = render_template(cmd, &vals(&[("user_id", "@a:s")]));
        assert_eq!(out, "users deactivate @a:s");
    }
}
